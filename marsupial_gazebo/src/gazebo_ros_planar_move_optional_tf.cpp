...
#include <gazebo_ros_planar_move_optional_tf.h>
...
namespace gazebo
{
      ...
      sdf::ElementPtr sdf)
  {
    ...
    else
    {
      ...
      boost::split( v_robot_base_frame_, robot_base_frame_, boost::is_any_of("::") );
      int s_ = v_robot_base_frame_.size();
      robot_base_frame_ = v_robot_base_frame_[s_ -1];
    }
    ...
    if (!sdf->HasElement("publishTf"))
    {
      ROS_WARN_NAMED("planar_move", "PlanarMovePlugin (ns = %s) missing <publishTf>, "
          "defaults to %s",
          robot_namespace_.c_str(), publish_odom_tf_?"true":"false");
    }
    else
    {
      publish_odom_tf_ = sdf->GetElement("publishTf")->Get<bool>();
    }
    marsupial_configuration_ = true;
    if (!sdf->HasElement("marsupialConfig"))
    {
      ROS_WARN_NAMED("planar_move", "PlanarMovePlugin (ns = %s) missing <marsupialConfig>, "
          "defaults to %s",
          robot_namespace_.c_str(), marsupial_configuration_?"true":"false");
    }
    else
    {
      marsupial_configuration_ = sdf->GetElement("marsupialConfig")->Get<bool>();
    }
  ...
  void GazeboRosPlanarMove::publishOdometry(double step_time)
  {
    ...
    if(publish_odom_tf_){
      if(marsupial_configuration_)
        transform_broadcaster_->sendTransform( tf::StampedTransform(base_footprint_to_odom, current_time, robot_namespace_+odom_frame, robot_namespace_ +base_footprint_frame));
      else
